...
def fetch_google_results(query, num_results=6, delay=3):
    results = []
    for result in google_search(query, num_results=num_results):
        results.append(result)
        time.sleep(delay)  # Introduce a delay between requestsS
    return results
...
def main():
    ...
    # Checkbox to store document text in input text area
    store_document_text = st.checkbox("Store Document Text in Input Text Area")
    ...
    if st.button("Check Plagiarism"):
        if uploaded_file is not None:
            ...
            if store_document_text:
                input_text = document_text  # Store document text in input text area
        else:
            document_text = input_text  # Use input text if no document uploaded
